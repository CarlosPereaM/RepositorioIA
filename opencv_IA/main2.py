...
import numpy as np
#CARGAMOS IMAGEN
imageG = cv2.imread('static/goku.jpg')
imageN = cv2.imread('static/naruto.png')
imagep = cv2.imread('static/pokemon.jpg')
imageR = cv2.imread('static/ranma.jpg')
imageSm = cv2.imread('static/sailormoon.jpg')
imageH = cv2.imread('static/harryp.jpg')
imageSC = cv2.imread('static/supercampenones.jpg')

cv2.imshow('Image', imageG)
cv2.waitKey(0)
cv2.destroyAllWindows()

#OBTENER DIMENSIONES DE LA IMAGEN
hight, width = imageN.shape[:2]
center = (width/2, hight/2)
#ROTAR LA IMAGEN
angulo = 90
matrix = cv2.getRotationMatrix2D(center, angulo, 1.0)
rotated = cv2.warpAffine(imageN, matrix, (width, hight))
...
translated = cv2.warpAffine(imagep, M, (imagep.shape[1], imagep.shape[0]))
...
scaled = cv2.resize(imageR, (new_width, new_height))
...
smoothed = cv2.GaussianBlur(imageSC, (5, 5), 0)
...
sharpened = cv2.filter2D(imageH, -1, kernel)
...
image = cv2.imread('static/goku.jpg', cv2.IMREAD_GRAYSCALE)
